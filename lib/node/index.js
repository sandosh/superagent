...
function request(method, url, options) {
  ...
  if ('function' == typeof url) {
    return new Request('GET', method, options).end(url);
  }

  // url first
  if (1 == arguments.length) {
    return new Request('GET', method, options);
  }
...
request.get = function(url, fn){
  var req = request('GET', url, this.options);
  if (fn) req.end(fn);
  return req;
};
...
request.post = function(url, data, fn){
  console.log(data);
  var req = request('POST', url, this.options);
